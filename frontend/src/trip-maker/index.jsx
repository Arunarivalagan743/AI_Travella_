...
function CreateTrip() {
  ...
  const login = useGoogleLogin({
    onSuccess: (response) => {
      ...
      // If there was a pending form submission, continue with it
      if (isFormComplete()) {
        handleFormSubmit();
      }
    },
  ...
  const onSubmit = async (e) => {
    ...
    if (!user) {
      // Show sign-in dialog if user is not logged in
      setOpenDialog(true);
      return;
    }
              ...
              {openDialog && (
                ...
                >
                  ...
                  >
                    ...
                    <motion.button
                      onClick={login}
                      className="w-full flex items-center justify-center gap-3 bg-white border border-gray-300 rounded-lg py-3 px-4 text-gray-700 hover:bg-gray-50 transition-colors duration-300 shadow-sm"
